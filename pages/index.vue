...
<template>
  <div class="flex flex-col min-h-0 overflow-auto overflow-x-hidden 2xl:px-8 px-6">
    <AppCard>
      <div class="flex items-start gap-4">

        <img class="w-10 h-10 bg-neutral-200 rounded-full" :src="getImgUrl(currentUser.photo)"
          :alt="currentUser.name"></img>
        <AppInputText class="flex-1" v-model="newPost" textarea placeholder="What are you thinking?" />
      </div>
      <div class="flex justify-between items-center mt-4">
        ...
        <AppBtn class="ml-auto flex gap-3" @click="addNewPost(newPost), (newPost = '')">
          Post
    ...
    <div
      class="my-8 2xl:my-10 text-xs 2xl:text-sm text-label dark:text-neutral-500 w-full h-[1px] bg-border dark:bg-neutral-600 flex items-center justify-center">
      <span class="bg-base dark:bg-neutral-800 px-6">Last posts</span>
    </div>

    <div class="flex flex-col gap-6 2xl:gap-8 flex-1 min-h-0">
      <AppPost v-for="(post, index) in posts" :key="post.id" :post="post" :last-in-list="index === posts.length - 1" />
    </div>
  </div>
</template>
